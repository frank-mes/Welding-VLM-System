...
with st.container(border=True):
    ...
    with c_up2:
        ...
        cp1.caption("电流 I 路径")
        cp1.write(f"`({res_calc['i_mat']}+12*{v_thick})*{res_calc['eta']}*{res_calc['delta']}`")
        cp2.caption("电压 U 路径")
        cp2.write(f"`{res_calc['u_b']}+{res_calc['u_adj']}+({final_i}/{res_calc['k']})`")
        cp3.caption("速度 V 路径")
        cp3.write(f"`(450-5.5*{v_thick})*{res_calc['eta']}*{res_calc['v_f']}`")

    # 核心看板 (Metrics)
    st.divider()
    r1, r2, r3, r4 = st.columns(4)
    r1.metric("推荐电流 (Current)", f"{final_i} A", delta=f"+{v_delta}A VLM" if v_delta > 0 else None)
    r2.metric("推荐电压 (Voltage)", f"{res_calc['u_res']} V")
    r3.metric("推荐速度 (Speed)", f"{res_calc['v_res']} mm/min")
    r4.metric("预测合格率 (Yield)", f"{res_calc['c_res']}%", delta_color="inverse")
    ...
    try:
        ...
        if len(event.selection.rows) > 0:
            ...
            with st.container(border=True):
                ...
                d1.write(f"**材料牌号**: {row_data['Material']}"); d1.write(f"**材料厚度**: {row_data['Thickness']} mm")
                d2.write(f"**焊接方法**: {row_data['Method']}"); d2.write(f"**质量等级**: {row_data['Grade']}")
                d3.write(f"**质检结果**: {row_data['Actual_Result']}"); d3.write(f"**专家评分**: {row_data['Expert_Score']}")
                st.divider()
