...
with st.expander("📘 全参数物理模型推导辞典 (1类常量 vs 2类变量)", expanded=True):
    ...
    # 定义全称标签 (纠偏版)
    C_SYS = "$\\color{blue}{1类常量：固定值(系统内置常数)}$"
    V_MAT = "$\\color{orange}{2类变量：随输入改变的变量(提示：与材料牌号输入有关)}$"
    V_THI = "$\\color{orange}{2类变量：随输入改变的变量(提示：与材料厚度输入有关)}$"
    V_MET = "$\\color{orange}{2类变量：随输入改变的变量(提示：与焊接方法输入有关)}$"
    V_GRA = "$\\color{orange}{2类变量：随输入改变的变量(提示：与质量等级输入有关)}$"
